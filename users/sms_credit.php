...
if($continue)
{
	...
	if($mmsel->count > 0)
	{
		$mmrow = mysql_fetch_row($mmsel->query);
		//high and low CAP
		if($quantity < $mmrow[0])
		{
			$val->error_code = 22;
...
if($xcredit != '')
	  {
		  while($prow = mysql_fetch_row($rsel->query))
		  {
...
$pay_row = @mysql_fetch_row($pay->query);
?>
<!DOCTYPE html>
<html lang="en">
  <head>
  ...
  else
  {
	  ...
	  if($success == '')
	  {
		  //get username
			...
			$fgetrow = mysql_fetch_row($fgetuser->query);
		  ?>
...
<?php
	  ...
	  elseif($option == 'bank')
	  {
		  ...
		  if($account->count > 0)
		  {
			  while($account_row = mysql_fetch_row($account->query))
			  {
		  ?>
