		...
		{*/
			...
			if($xsend->success_code > 0)
			{
				...
				if($xsend->success_code > 0)
				{
					//log
					$rin = new insert();
					$rin->input('smslog', 'id, senderid, destination, message, credit, user, date', "0, '$info_row[0]', '$otell', '$message', $ncount, $auser, '$now'");
...
if($save)
{
	...
	{
		$check = new select();
		$check->pick('card', '*', 'id,user', "$xedit,$auser", '', 'record', '', '', '=,=', 'and');
		if($check->count < 1)
		{
	...
	if($sval->error_code < 1)
	{
		...
		{
		$in = new insert();
		$in->input('card', 'id, title, name, tell, url, company, description, user', "0, '$title', '$name', '$tell', '$url', '$company', '$description', $auser");
		}
		else
		{
			$in = new update();
...
{
		$check = new select();
		$check->pick('card', '*', 'id,user', "$delete,$auser", '', 'record', '', '', '=,=', 'and');
		if($check->count < 1)
		{
			header("location: card.php");
			exit;
 ...
 ?>
  ...
  <?php
	...
	else
	{
		...
		//get cards
		$gc = new select();
		$gc->pick('card', 'id, title, name, tell, url, company, description', 'user', "$auser", '', 'record', 'title', '', '=', '');
		if($gc->count > 0)
		{
			?>
