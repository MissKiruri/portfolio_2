...
if($delete != '')
{
	...
	$ups = new update();
	$ups->up('user', 'reserved', 'id', "$auser", "reserved - $dsel_row[0]");
...
<html lang="en">
  ...
  $sel = new select();
	$sel->pick('schedule', 'id, senderid, message, unix_timestamp(entrydate), unix_timestamp(senddate)', 'user', "$auser", '', 'record', 'id desc', '', '=', '');
